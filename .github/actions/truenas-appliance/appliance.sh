...
#
# Every call a workflow makes against appliance infrastructure goes through
# this file. One place, so the contract with the lab is written down rather
# than spread across YAML, and so the day that contract changes is a one-file
# change. It is the body of the `truenas-appliance` action beside it: the
# action's main step runs `iso` then `claim`, and its post step runs
# `release` whatever happened in between. It also runs by hand, with the
...
claim() {
  ...
  # Expired leases go first. The lease is the guarantee behind every release
  # path — a runner that died mid-job never ran its post step — and this is
  # where it is enforced when no cron on the host does it. Best effort: a
  # prune that cannot run must not stop a claim, and it says so in the log.
